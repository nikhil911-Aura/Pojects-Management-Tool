...
// Build karya-style suggestions from the typed input. If the user typed a
...
/**
 * TimeTracker — karya-style time tracking cell
  ...
  return (
    <div className="relative">
      ...
      <button ref={btnRef} onClick={(e) => { e.stopPropagation(); handleOpen(); }}
        disabled={!canEdit}
        className={`text-xs flex items-center w-full ${!canEdit ? 'cursor-default pointer-events-none' : ''} ${cellText || timerStart ? 'text-[var(--karya-text-primary)]' : 'text-[var(--karya-text-secondary)] opacity-0 group-hover:opacity-100'}`}>
        {timerStart && (
      ...
      {showPopup && (
        <div ref={popupRef} className="fixed z-[200] w-72 bg-[var(--karya-surface)] border border-[var(--karya-border)] rounded-xl shadow-2xl animate-fade-in"
          ...
          <div className="max-h-48 overflow-y-auto">
            {(entries || []).map(entry => (
              <div key={entry.id} className="flex items-center px-4 py-2.5 border-b border-[var(--karya-border)] hover:bg-gray-50 dark:hover:bg-gray-800/30 group/entry">
                {editingId === entry.id ? (
                  <input type="text" value={editInput} onChange={(e) => setEditInput(e.target.value)} autoFocus
                    className="text-sm font-semibold bg-transparent border-b border-karya-blue outline-none text-[var(--karya-text-primary)] w-20"
                  ...
                  <span className="text-sm font-semibold text-[var(--karya-text-primary)] flex-1">{formatMins(entry.minutes)}</span>
                )}
                  ...
                  {canEdit && editingId !== entry.id && (
                    <div className="flex items-center space-x-1 opacity-0 group-hover/entry:opacity-100">
                      <button onClick={() => { setEditingId(entry.id); setEditInput(formatMins(entry.minutes)); }}
                        className="p-0.5 rounded hover:bg-gray-200 dark:hover:bg-gray-700 text-[var(--karya-text-secondary)]">
                        <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                      </button>
                      <button onClick={() => handleDeleteEntry(entry.id, entry.minutes)}
                        className="p-0.5 rounded hover:bg-red-100 dark:hover:bg-red-900/30 text-[var(--karya-text-secondary)] hover:text-red-500">
                        <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                      </button>
                    </div>
                  )}
                  <span className="text-[10px] text-[var(--karya-text-secondary)]">
            ...
            {entries?.length === 0 && !timerStart && (
              <p className="text-xs text-[var(--karya-text-secondary)] text-center py-4">No time logged yet</p>
            )}
          </div>
          ...
          <div className="px-4 py-3">
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm font-semibold text-[var(--karya-text-primary)]">
                {formatMins(total)}{timerStart && <span className="text-red-500 ml-1">+ {formatMins(timerMins)}</span>}
                <span className="text-[10px] font-normal text-[var(--karya-text-secondary)] uppercase ml-1.5">Total</span>
              </span>
            </div>
              ...
              <div className="flex items-center space-x-2">
                {!timerStart ? (
                  <button onClick={handleStartTimer}
                    className="flex items-center text-xs px-3 py-1.5 rounded-md border border-[var(--karya-border)] text-[var(--karya-text-primary)] hover:bg-gray-50 dark:hover:bg-gray-800 font-medium">
                ...
                {addingTime ? (
                  <div className="relative flex items-center space-x-1">
                    ...
                    {/* Suggestion dropdown — karya-style. Appears as the user
            ...
            {canEdit && !addingTime && (
              <div className="mt-3 pt-3 border-t border-[var(--karya-border)]">
                <p className="text-[10px] uppercase tracking-wider text-[var(--karya-text-secondary)] font-semibold mb-1.5">
                  Quick add
                </p>
                <div className="flex flex-wrap gap-1.5">
                  {[
                    ...
                    <button
                      ...
                      className="px-2 py-1 text-[11px] rounded-md bg-gray-100 dark:bg-gray-800 text-[var(--karya-text-primary)] hover:bg-karya-blue hover:text-white transition-colors font-medium"
                    >
                      +{preset.label}
                    </button>
                  ))}
