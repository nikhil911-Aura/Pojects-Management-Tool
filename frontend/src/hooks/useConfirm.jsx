...
function ConfirmModalInline({
  ...
  const btnColorMap = {
    ...
    info: 'bg-karya-blue hover:brightness-110',
  };

  return (
    <div className="fixed inset-0 z-[300] flex items-center justify-center bg-black/50 animate-fade-in" onClick={onCancel}>
      <div className="bg-[var(--karya-surface)] rounded-xl shadow-2xl border border-[var(--karya-border)] w-full max-w-sm mx-4" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex items-center border-t border-[var(--karya-border)] divide-x divide-[var(--karya-border)]">
          <button onClick={onCancel}
            className="flex-1 px-4 py-3 text-sm font-medium text-[var(--karya-text-secondary)] hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors rounded-bl-xl">
            {cancelText}
          </button>
